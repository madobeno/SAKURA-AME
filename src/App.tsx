...
const App: React.FC = () => {
  ...
  return (
    <div
      className="fixed inset-0 h-[100svh] w-full flex flex-col items-center justify-center bg-stone-950 text-sakura-100 overflow-hidden"
    ...
    >
      ...
      <div className="z-10 text-center space-y-8 p-12 max-w-lg bg-stone-950/20 backdrop-blur-3xl rounded-3xl border border-white/5 shadow-[0_35px_60px_-15px_rgba(0,0,0,0.6)] animate-ripple-in">
        <h1 className="text-7xl sm:text-8xl font-serif tracking-[0.4em] text-white mb-2 drop-shadow-[0_15px_15px_rgba(0,0,0,0.9)]">
  桜雨
</h1>
        <h2 className="text-xl font-light tracking-[0.3em] text-sakura-100/90 uppercase">
          Sakura Ame
        </h2>
        <div className="w-24 h-[1.5px] bg-white/40 mx-auto my-6" />
        <p className="text-sm font-serif text-white/80 italic tracking-widest">
          Gentle Rain Instrument
        </p>
        <div className="mt-8 px-14 py-4 border border-white/30 bg-white/5 rounded-full text-xs text-white font-bold tracking-[0.4em] uppercase">
          Start Experience
        </div>
      </div>
    </div>
  ...
  return (
    <div className={`relative h-screen w-full bg-stone-950 overflow-hidden font-serif select-none transition-colors duration-1000 ${activeEffect === 'storm' ? 'animate-pulse-slow' : ''}`} onMouseDown={() => audioEngine.resume()} onTouchStart={() => audioEngine.resume()}>
